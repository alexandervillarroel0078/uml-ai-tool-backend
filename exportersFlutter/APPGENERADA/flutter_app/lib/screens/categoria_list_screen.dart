...
class _CategoriaListScreenState extends State<CategoriaListScreen> {
  ...
  Future<void> fetchData() async {
    final response = await ApiService().getData('api/categorias');
    setState(() {
      items = response ?? [];
      loading = false;
    });
  ...
  Future<void> deleteItem(int id) async {
    await ApiService().deleteData('api/categorias/$id');
    fetchData();
  }
  ...
  String buildSubtitle(Map<String, dynamic> item) {
    ...
    if (item['categoria'] != null) {
      final relItem = item['categoria'];
      final label = relItem['nombre'] ?? relItem['titulo'] ?? 'ID: ${relItem['id']}';
      buffer.writeln('Categoria: $label');
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await Navigator.pushNamed(context, '/categoriaDetail');
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            final fullData = await ApiService().getOne("api/categorias/${item["id"]}");
