...
class _CursoListScreenState extends State<CursoListScreen> {
  ...
  Future<void> fetchData() async {
    final response = await ApiService().getData('api/cursos');
    setState(() {
      items = response ?? [];
      loading = false;
    });
  ...
  Future<void> deleteItem(int id) async {
    await ApiService().deleteData('api/cursos/$id');
    fetchData();
  }
  ...
  String buildSubtitle(Map<String, dynamic> item) {
    ...
    if (item['nivel'] != null) {
      buffer.writeln('Nivel: ${item['nivel']}');
    }
    
    if (item['duracion'] != null) {
      buffer.writeln('Duracion: ${item['duracion']}');
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await Navigator.pushNamed(context, '/cursoDetail');
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            final fullData = await ApiService().getOne("api/cursos/${item["id"]}");
