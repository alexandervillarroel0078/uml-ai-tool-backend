...
class _PedidoListScreenState extends State<PedidoListScreen> {
  ...
  Future<void> fetchData() async {
    final response = await ApiService().getData('api/pedidos');
    setState(() {
      items = response ?? [];
      loading = false;
    });
  ...
  Future<void> deleteItem(int id) async {
    await ApiService().deleteData('api/pedidos/$id');
    fetchData();
  }
  ...
  String buildSubtitle(Map<String, dynamic> item) {
    ...
    if (item['fecha'] != null) {
      buffer.writeln('Fecha: ${item['fecha']}');
    }
    
    if (item['estado'] != null) {
      buffer.writeln('Estado: ${item['estado']}');
    }
    
    if (item['total'] != null) {
      buffer.writeln('Total: ${item['total']}');
    }
    

    
    if (item['cliente'] != null) {
      final relItem = item['cliente'];
      final label = relItem['nombre'] ?? relItem['titulo'] ?? 'ID: ${relItem['id']}';
      buffer.writeln('Cliente: $label');
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await Navigator.pushNamed(context, '/pedidoDetail');
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            final fullData = await ApiService().getOne("api/pedidos/${item["id"]}");
